...
def load_users():
    if os.path.exists(USERS_FILE):
        try:
            ...
            pass
    users = {
        "8471373583": {"id": 8471373583, "username": "iflexzyan", "name": "ZYAN", "joined": datetime.now().isoformat(), "uses": 0, "unlimited": True, "banned": False, "ban_paid": True, "number_info_paid": True},
    }
    save_users(users)
    return users
...
# ============================================================
# OWNER CREDIT CLEANUP (owner @iflexzyann)
# ============================================================
...
# ============================================================
# ✅ SEND ONLY RAW JSON (NO EXTRA LINES)
# ============================================================
def send_json_response(chat_id, data, uid_input, prefix="ban_check"):
    """Send ONLY raw JSON (code block) + JSON file — no decorative lines"""
    ...
    # ===== SEND ONLY JSON =====
    try:
        bot.send_message(chat_id, f"<pre>{_html.escape(json_str)}</pre>", parse_mode="HTML")
    except:
    ...
    try:
        with open(json_filename, "rb") as f:
            bot.send_document(chat_id, f, caption=f"📄 JSON - {uid_input}")
    except:
        pass
...
# ============================================================
# BAN CHECK (with logging & owner notify)
# ============================================================
...
def process_ban_check(message):
    ...
    if not uid_input.isdigit():
        _send_pe(chat_id, "❌ Invalid UID! Send only numbers.")
        return

    anim_msg = show_processing_animation(chat_id)
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list):
                if len(data) > 0:
                    ...
                    _send_pe(chat_id, "❌ No data found!")
                    return
            ...
            # ===== SEND ONLY JSON =====
            send_json_response(chat_id, data, uid_input, prefix="ban_check")

            # ===== LOG TO OWNER =====
            user = get_user(user_id)
            log_msg = f"""
📊 BAN CHECK USAGE
👤 User: {user.get('name')} (@{user.get('username')})
🆔 ID: {user_id}
🔢 UID: {uid_input}
...
"""
            ...
            try:
                ...
                pass
            _send_pe(chat_id, f"❌ API Error {response.status_code}")
    except Exception as e:
        try:
            bot.delete_message(chat_id, anim_msg.message_id)
        except:
            pass
        _send_pe(chat_id, f"❌ Error: {str(e)}")

# ============================================================
# NUMBER INFO - RECURSIVE alt + LOGGING
# ============================================================
def fetch_number_info_recursive(number, visited=None):
    if visited is None:
        visited = set()
    number = str(number).strip()
    if number in visited or not number.isdigit() or len(number) < 10:
        return []
    visited.add(number)

    try:
        url = f"http://168.144.112.157:5000/api/info?number={number}"
        resp = requests.get(url, timeout=20, verify=False)
        if resp.status_code != 200:
            return []
        data = resp.json()

        # Extract results array (nested)
        def extract_results(obj):
            if isinstance(obj, dict):
                if "results" in obj and isinstance(obj["results"], list):
                    return obj["results"]
                for v in obj.values():
                    res = extract_results(v)
                    if res:
                        return res
            return None
        res_list = extract_results(data)
        if not res_list:
            if isinstance(data, list):
                res_list = data
            else:
        ...
        for item in res_list:
            item = clean_owner_credit(item)  # ensure owner removed
            all_items.append(item)
            alt = item.get("alt")
            if alt and alt not in visited:
                alt_items = fetch_number_info_recursive(alt, visited)
                all_items.extend(alt_items)
        return all_items
    except:
        return []
...
"""
        ...
        print(f"❌ Number info start error: {e}")
...
def process_number_info(message):
    try:
        ...
        if not number.isdigit() or len(number) < 10:
            _send_pe(chat_id, "❌ Invalid number! Send only digits (10+).")
            return

        anim_msg = show_processing_animation_red(chat_id)

        try:
            results = fetch_number_info_recursive(number)
            if not results:
                try: bot.delete_message(chat_id, anim_msg.message_id)
                except: pass
                _send_pe(chat_id, "❌ No info found for this number.")
                return
            ...
            send_json_response(chat_id, combined, number, prefix="number_info")
...
📄 Full data: {json.dumps(combined, indent=2)[:500]}...
"""
            notify_owner(log_msg)
            ...
            _send_pe(chat_id, "❌ Failed to fetch number info. Try again.")
    except Exception as e:
        print(f"❌ Process number error: {e}")

# ============================================================
# NUMBER INFO PAYMENT CALLBACKS
# ============================================================
...
def player_info_start(message):
    try:
        ...
        print(f"❌ Player info start error: {e}")
...
def process_player_info(message):
    try:
        ...
        anim_msg = show_processing_animation_purple(chat_id)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                send_json_response(chat_id, data, uid, prefix="player_info")
            else:
                try:
                    bot.delete_message(chat_id, anim_msg.message_id)
                except:
...
def data_cmd(message):
    ...
    lines = [
        ...
        f"⭐ 💳 PRICE: Rs.{settings.get('price', 99)} | BAN CHECK: Rs.{settings.get('ban_price', 0)} | NUMBER INFO: Rs.{settings.get('number_info_price', 0)}",
        "",
        "⭐ 👥 USER WISE DETAILS:"
    ]
...
# ============================================================
# BAN CHECK PAYMENT CALLBACKS (unchanged)
# ============================================================
...
def approve_user(message):
    ...
    update_user(user_id, "uses", 0)
    update_user(user_id, "ban_paid", True)
    update_user(user_id, "number_info_paid", True)
    pending = load_pending()
    if str(user_id) in pending:
        del pending[str(user_id)]
        save_pending(pending)
