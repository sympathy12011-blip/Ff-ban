...
# ============================================================
# OWNER CREDIT CLEANUP (🔴 FIXED: owner ko @iflexzyann)
# ============================================================
def clean_owner_credit(data):
    ...
    if isinstance(data, dict):
        ...
        data["owner"] = "@iflexzyann"   # 🔴 CHANGE YAHAN
    return data
...
# ============================================================
# ✅ NUMBER INFO - FIXED (SAHI URL + ERROR HANDLING)
# ============================================================
...
def process_number_info(message):
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                data = clean_owner_credit(data)   # owner -> @iflexzyann
                ...
                try:
                    ...
                    pass
                _send_pe(message.chat.id, "❌ Number info not found. Please check the number.")
        except requests.exceptions.Timeout:
            try:
                bot.delete_message(message.chat.id, anim_msg.message_id)
            except:
                pass
            _send_pe(message.chat.id, "❌ Request timeout. Try again later.")
        except requests.exceptions.ConnectionError:
            try:
                bot.delete_message(message.chat.id, anim_msg.message_id)
            except:
                pass
            _send_pe(message.chat.id, "❌ Connection error. Please try again.")
        except Exception as e:
            try:
                bot.delete_message(message.chat.id, anim_msg.message_id)
            except:
                pass
            _send_pe(message.chat.id, "❌ Failed to fetch number info. Try again.")
    except Exception as e:
